...
function resolveOutputLanguage(state, options = {}) {
  const candidates = [
    [
      "tool_selected",
      options.toolSelectedLanguage || state?.toolSelectedLanguage,
    ],
...
export function createMetaRunStatusEnvelope(state, options = {}) {
  ...
  const nextStage = stageIndex < stageTotal ? STAGE_ORDER[stageIndex] : null;
  const startedAt =
  ...
  return {
    ...
    active,
    runId,
    triggeredBy: state?.triggerReason || state?.triggeredBy || "meta-theory",
...
export async function readMetaRunStatus(cwd, profile) {
  const filePath = runStatusPaths(
    cwd,
    sanitizeStateProfile(profile),
    "latest",
  ).activeRun;
  try {
    const raw = await readFile(filePath, "utf-8");
...
export function recordIntentConfirmation(
  state,
  confirmationState,
  correctionPayload,
) {
...
function hasExecutionOwnerEvidence(state) {
  if (
    firstString(state?.dispatchBoard?.ownerAgent, state?.dispatchBoard?.owner)
  ) {
    return true;
  }
  if (hasNonEmptyArray(state?.workerTaskPackets)) {
...
function hasLoadoutEvidence(state) {
  ...
  if (hasNonEmptyArray(state?.workerTaskPackets)) {
    return state.workerTaskPackets.some(
      (packet) =>
  ...
  if (hasNonEmptyArray(state?.agentBlueprintPacket?.roles)) {
    return state.agentBlueprintPacket.roles.some(
      (role) =>
